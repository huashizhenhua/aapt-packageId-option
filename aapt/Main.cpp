    ...
    while (argc && argv[0][0] == '-') {
        ...
        while (*cp != '\0') {
            ...
            case 'g':
                ...
                break;
            // -y {packageID} .e.g -y  127
            case 'y':
                argc--;
                argv++;
                if (!argc) {
                    fprintf(stderr, "ERROR: No argument supplied for '-y' option\n");
                    wantUsage = true;
                    goto bail;
                }
                bundle.setPackageResId(atoi(argv[0]));
                printf("%s: packageResId = %d\n", prog, atoi(argv[0]));
                break;
            // -h {rJavaFileName} .e.g -h
            case 'h':
                argc--;
                argv++;
                if (!argc) {
                    fprintf(stderr, "ERROR: No argument supplied for '-h' option\n");
                    wantUsage = true;
                    goto bail;
                }
                bundle.setRJavaFileName(argv[0]);
                printf("%s: rJavaFileName = %s\n", prog, argv[0]);
                break;
            case 'k':
